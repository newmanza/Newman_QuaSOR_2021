...
%   Stack_Viewer(ImageArray,DataRegion,StackOrder,Channel_Labels,Channel_Colors,Channel_Info,ImagingInfo,SaveName,InputAnalysis,ReleaseFig)
...
% 2D: 'YX'
% 3D: 'YXT';'YXZ';'YXC'
% 4D: 'YXZT';'YXTZ';'YXCT';'YXTC';'YXZC';'YXCZ';'YX[RGB]T';'YXT[RGB]';'YX[RGB]Z';'YXZ[RGB]
% 5D: 'YXTZC';'YXZTC';'YX[RGB]ZC'
...
% Dependencies: Bioformats export_fig
    ...
    if ~isempty(Channel_Labels)&&~isempty(Channel_Info)
        if length(Channel_Labels)~=length(Channel_Info)
            warning('Channel_Label and Channel_Info Size Mismatch! Using Channel_Labels')
            Channel_Info=Channel_Info(1:length(Channel_Labels));
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %Few Starting parameters
        ...
        StackOrderOptions2={'YX'};
        StackOrderOptions3={'YXT';'YXZ';'YXC'};
        StackOrderOptions4={'YXZT';'YXTZ';'YXCT';'YXTC';'YXZC';'YXCZ';'YX[RGB]T';'YXT[RGB]';'YX[RGB]Z';'YXZ[RGB]'};
        StackOrderOptions5={'YXTZC','YXZTC','YX[RGB]ZC'};
        
        TraceParam.TraceType=1;%1=Mean 2=DeltaF 3=DeltaF/F
        TraceParam.TraceTypeOptions={'Mean Intensity','Mean DeltaF','Mean DeltaF/F'};
        TraceParam.TraceTypeLabel=TraceParam.TraceTypeOptions{TraceParam.TraceType};
        TraceParam.F0_Method=1;%1-4
        TraceParam.F0_MethodOptions={'Fixed Mean','Fixed Min','Running Mean','Running Min'};
        TraceParam.F0_MethodLabel=TraceParam.F0_MethodOptions{TraceParam.F0_Method};
        TraceParam.F0_Frames=[1];
        TraceParam.F0_FrameWidth=11;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for zzzz=1:1
        ...
        if isempty(StackOrder)
            switch length(ImageArrayDimensions)
                case 2
                    [StackOrderChoice, ~] = listdlg('PromptString',Question,'SelectionMode','single','ListString',StackOrderOptions2,'ListSize', [200 200]);
                    StackOrder=StackOrderOptions2{StackOrderChoice};
                case 3
        ...
        switch StackOrder
            case 'YX'
                ...
                Slice=1;
                Frame=1;
                Channel_Info(Channel).Whole_Image_Values=[];
                ...
                Channel_Info(Channel).Whole_Image_Values=nanmean(TempImage(:));
                Channel_Info(Channel).Slice(Slice).Whole_Image_Values=nanmean(TempImage(:));
            ...
            case 'YXT'
                ...
                for t=1:Last_T
                    ...
                    Channel_Info(Channel).Whole_Image_Values(t)=nanmean(TempImage(:));
                    Channel_Info(Channel).Slice(Slice).Whole_Image_Values(t)=nanmean(TempImage(:));
            ...
            case 'YXC'
                ...
                Slice=1;
                for c=1:Last_C
                    Channel_Info(c).Whole_Image_Values=[];
            ...
            case 'YXZT'
                ...
                for t=1:Last_T
                    ...
                    Channel_Info(c).Whole_Image_Values(t)=nanmean(TempStack(:));
                    for z=1:Last_Z
                        ...
                        Channel_Info(c).Slice(z).Whole_Image_Values(t)=nanmean(TempImage(:));
                        count=count+1;
            ...
            case 'YXTC'
                ...
                for c=1:Last_C
                    ...
                    for t=1:Last_T
                        ...
                        Channel_Info(c).Whole_Image_Values(t)=nanmean(TempImage(:));
                        Channel_Info(c).Slice(Slice).Whole_Image_Values(t)=nanmean(TempImage(:));
                        if any(t==[1:round(Last_T/TextUpdateIntervals):Last_T])
                            fprintf('.')
                        end
            ...
            case 'YXZC'
                ...
                for c=1:Last_C
                    ...
                    for z=1:Last_Z
                        ...
                        Channel_Info(c).Whole_Image_Values(z)=nanmean(TempImage(:));
                        Channel_Info(c).Slice(z).Whole_Image_Values=nanmean(TempImage(:));
                        if any(z==[1:round(Last_Z/TextUpdateIntervals):Last_Z])
                            fprintf('.')
                        end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            %         end
                    ...
                    Channel_Info(cc).DisplayColorMapIndex=1;
                    Channel_Info(cc).DisplayColorMap=[];
                    Channel_Info(cc).DisplayColorMapCode=[];
                    Channel_Info(cc).DisplayColorMapIndex=0;
                    Channel_Info(cc).Display_Limits=[0,1];
                    Channel_Info(cc).Normalized_Display_Limits=...
                        (Channel_Info(cc).Display_Limits-...
                        Channel_Info(cc).DisplayMinVal)/Channel_Info(cc).DisplayValDiff;
                    Channel_Info(cc).Normalized_StepUnits=(Channel_Info(cc).StepUnits)/Channel_Info(cc).DisplayValDiff;
...
%                 Channel_Info(cc).EditRecordChannel=0;
        ...
        if ~Z_Stack&&~isfield(ImagingInfo,'VoxelDepth')
            ImagingInfo.VoxelDepth=0;
            ImagingInfo.VoxelUnit='um';
        end
        if ~T_Stack&&~isfield(ImagingInfo,'InterFrameTime')
...
set(ClearProfileButton,'Enable','off');
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %Trace Controls
            TraceJumpButton = uicontrol('Style', 'pushbutton', 'String', {'TJump'},...
                'units','normalized',...
                'Position', [0 0.18 0.02 0.02],....
                'Callback', @TraceJump2Frame);
            TraceTypeButton = uicontrol('Style', 'pushbutton', 'String', {'TraceType'},...
                'units','normalized',...
                'Position', [0 0.02 0.04 0.02],...
                'Callback', @TraceTypeAdjust);
            ...
            FrameMarkerLabelsButton = uicontrol('Style', 'togglebutton', 'String', 'MarkerLabel',...
                ...
                'units','normalized',...
                'Position', [0.17 0 0.05 0.02],...
                'Callback', @FrameMarkerLabelsToggle);
            WholeImageTraces_Button = uicontrol('Style', 'togglebutton', 'String', 'WholeImageTraces',...
                'value',WholeImageTraces,...
                'units','normalized',...
                'Position', [0.22 0 0.05 0.02],...
                'Callback', @DisplayWholeImageTraces);
            SliceWholeImageTraces_Button = uicontrol('Style', 'togglebutton', 'String', 'SliceTraces',...
                'value',SliceWholeImageTraces,...
                'units','normalized',...
                'Position', [0.27 0 0.05 0.02],...
                'Callback', @DisplaySliceWholeImageTraces);
            FormatTraces_Button = uicontrol('Style', 'pushbutton', 'String', 'FormatTrace',...
                'units','normalized',...
                'Position', [0.32 0 0.05 0.02],...
                'Callback', @FormatTraces);
            FormatFrameMarkers_Button = uicontrol('Style', 'pushbutton', 'String', 'FormatMarkers',...
                'units','normalized',...
                'Position', [0.37 0 0.05 0.02],...
                'Callback', @FormatFrameMarkers);
            FormatROIs_Button = uicontrol('Style', 'pushbutton', 'String', 'FormatROI',...
                'units','normalized',...
                'Position', [0.42 0 0.05 0.02],...
                'Callback', @FormatROIs);
            ROITraces_Button = uicontrol('Style', 'togglebutton', 'String', 'ROIs',...
                'value',ROITraces,...
                'units','normalized',...
                'Position', [0.47 0 0.02 0.02],...
        ...
        %ROI controls 
            ResetTraces_Button = uicontrol('Style', 'pushbutton', 'String', 'Reset',...
                'units','normalized',...
                'Position', [0.49 0 0.03 0.02],...
                'Callback', @ResetTrace);   
            UpdateTrace_Pixel_Button = uicontrol('Style', 'pushbutton', 'String', '+Pixel',...
                'units','normalized',...
                'Position', [0.52 0 0.03 0.02],...
                'Callback', @AddPixelTrace);   
            UpdateTrace_ROI_Button = uicontrol('Style', 'pushbutton', 'String', '+ROI',...
                'units','normalized',...
                'Position', [0.55 0 0.03 0.02],...
                'Callback', @AddROITrace);
            Delete_ROI_Button = uicontrol('Style', 'pushbutton', 'String', '-ROI',...
                'units','normalized',...
                'Position', [0.58 0 0.02 0.02],...
                'Callback', @DeleteROI);
            Edit_ROI_Button = uicontrol('Style', 'pushbutton', 'String', 'EdROI',...
                'units','normalized',...
                'Position', [0.60 0 0.03 0.02],...
                'Callback', @EditROI);
            UndoROI_Button = uicontrol('Style', 'pushbutton', 'String', 'Undo',...
                'units','normalized',...
                'Position', [0.63 0 0.02 0.02],...
                'Callback', @UndoROI);
            ZoomXTraces_Button = uicontrol('Style', 'pushbutton', 'String', 'ZoomX',...
                'units','normalized',...
                'Position', [0.65 0 0.03 0.02],...
                'Callback', @ZoomXTrace);
            ZoomYTraces_Button = uicontrol('Style', 'pushbutton', 'String', 'ZoomY',...
                'units','normalized',...
                'Position', [0.68 0 0.03 0.02],...
                'Callback', @ZoomYTrace);
            ResetZoomTraces_Button = uicontrol('Style', 'pushbutton', 'String', 'ResetZoom',...
                'units','normalized',...
                'Position', [0.71 0 0.04 0.02],...
...
set(ResetZoomTraces_Button,'Enable','off');
            if ~T_Stack||RGB_Stack
                set(TraceJumpButton,'Enable','off');
                set(TraceTypeButton,'Enable','off');
                ...
                set(WholeImageTraces_Button,'Enable','off','value',0);
                ...
                set(SliceWholeImageTraces_Button,'Enable','off','value',0);
...
set(SliceColorButton,'Enable','off');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function DataOutput
            ...
            if NumOutputArgs>=4
                if ~isempty(ROIs)||~isempty(Locations)||~isempty(EditRecord)
                    warning(['Analyses were Performed! Do you want to SAVE the OutputAnalysis?'])
                    SaveAnalysis = questdlg({['Analyses were Performed!'];'Do you want to SAVE or CLEAR OutputAnalysis?'},...
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function CurrentImages=FindCurrentImage(Temp_Channel_Input,Temp_Frame_Input,Temp_Slice_Input,StackOrder,...
            ...
            for i=1:Images
                ...
                if Temp_Channel==0&&Temp_MergeChannel==0
                    Temp_MergeChannel=1;
                end
                ...
                if ~RGB_Stack
                    ...
                    if Temp_Z_Projection&&~Temp_T_Projection
                        if ~Temp_MergeChannel||Temp_OverwriteMerge||any(Temp_Channel==[-11:-1:-19])
                            if any(Temp_Channel==[-11:-1:-19])
                                Temp_Channel=abs(Temp_Channel+10);
                            end
                    ...
                    elseif Temp_T_Projection&&~Temp_Z_Projection
                        if ~Temp_MergeChannel||Temp_OverwriteMerge||any(Temp_Channel==[-21:-1:-29])
                            if any(Temp_Channel==[-21:-1:-29])
                                Temp_Channel=abs(Temp_Channel+20);
                            end
...
%                         alpha(ForegroundLayer(i),MaskAlpha)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function [EditRegion,EditData,EditRegionBorderLine]=ApplyEdit(EditRegion,Channel,Frame,Slice,StackOrder,EditMode)
            ...
            Channel_Info(Channel).Whole_Image_Values(Frame)=nanmean(CurrentImage(:));
            Channel_Info(Channel).Slice(Slice).Whole_Image_Values(Frame)=nanmean(CurrentImage(:));
...
%                             end
        ...
        function UndoEdit(~,~,~)
            ...
            if Channel_Info(Channel).Editable
                if ~isempty(EditRecord)
                    ...
                    CurrentTraceYData=Channel_Info(Channel).Whole_Image_Values;
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %UpdateDisplay
                    %SetColorMap;
                    ColorBarDisplay(ViewerFig,ColorBarAxis,ColorBarAxisPosition);
    ...
    %                                 end
                ...
                warning on
                warning('Problem Updating Histogram')
            end
        end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function TraceTypeAdjust(~,~,~)
            WasPlaying=PlayBack;
            if WasPlaying
                PausePlayStack(PauseButton);
            end
            ...
            set(TraceTypeButton, 'Enable', 'off');
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            Question='How would you like to calculate the trace values?';
            [TraceParam.TraceType, ~] = listdlg('PromptString',Question,'SelectionMode','single','ListString',TraceParam.TraceTypeOptions,'ListSize', [200 200],'InitialValue',TraceParam.TraceType);
            TraceParam.TraceTypeLabel=TraceParam.TraceTypeOptions{TraceParam.TraceType};
            if TraceParam.TraceType==1
                CheckF0=0;
            else
                CheckF0=1;
            end
            if CheckF0
                Question='How would you like to calculate Baseline/F0?';
                [TraceParam.F0_Method, ~] = listdlg('PromptString',Question,'SelectionMode','single','ListString',TraceParam.F0_MethodOptions,'ListSize', [200 200],'InitialValue',TraceParam.F0_Method);
                TraceParam.F0_MethodLabel=TraceParam.F0_MethodOptions{TraceParam.F0_Method};
                if TraceParam.F0_Method==1||TraceParam.F0_Method==2

                    FrameOptions=[];
                    for tt=1:Last_T
                        FrameOptions{tt}=num2str(tt);
                    end
                    [TraceParam.F0_Frames, checking] = listdlg('PromptString',{'Select T Range for Baseline/F0 calculation'},...
                        'SelectionMode','multiple','ListString',FrameOptions,'ListSize', [200 600],'InitialValue',TraceParam.F0_Frames);

                elseif TraceParam.F0_Method==3||TraceParam.F0_Method==4

                    prompt = {  'Num Frames for Running Mean/Min',...
                    };
                    dlg_title = 'Running Baseline Setting';
                    num_lines = 1;
                    def = {...
                            num2str(TraceParam.F0_FrameWidth)...
                            };
                    answer = inputdlg(prompt,dlg_title,num_lines,def);
                    TraceParam.F0_FrameWidth=           str2num(answer{1});
                    clear answer
            ...
            warning('NOT YET Recalculating Overall Image Values...')
%             
%             fprintf('Recalculating Overall Image Values...')
%             [TraceChannel]=ExtractTraces(TraceParam,2,[],DataRegion,[]);
%             for ccc=1:Last_C
%                 Channel_Info(ccc).Whole_Image_Values=TraceChannel(ccc).Trace;
%                 Channel_Info(ccc).Slice(Slice).Whole_Image_Values=TraceChannel(ccc).Trace;
%             end
%             clear TraceChannel
%             if Z_Stack
%                 fprintf('Recalculating Overall Slice Image Values...')
%                 for zzz=1:Last_Z
%                     [TraceChannel]=ExtractTraces(TraceParam,2,[],DataRegion,zzz);
%                     for ccc=1:Last_C
%                         Channel_Info(ccc).Slice(zzz).Whole_Image_Values=TraceChannel(ccc).Trace;
%                     end
%                 end
%             end
%             clear TraceChannel            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if ~isempty(ROIs)
                fprintf('Recalculating ROI Traces...')
                for r=1:length(ROIs)
                    if ROIs(r).Type==1
                        [ROIs(r).TraceChannel]=ExtractTraces(TraceParam,ROIs(r).Type,ROIs(r).Coord,[],ROIs(r).Slice);
                    else
                        [ROIs(r).TraceChannel]=ExtractTraces(TraceParam,ROIs(r).Type,[],ROIs(r).DataRegion,ROIs(r).Slice);
                    end
                end
                fprintf('Finished!\n')
            end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function TraceDisplay(TraceLim,Channel,Frame,CurrentSlices,CurrentFig,CurrentTracePlotAxis,CurrentTraceAxisPosition,SplitTraceAdjust)
            if T_Stack
                    ...
                    if WholeImageTraces
                        if MergeChannel
                            ...
                            for c1=1:length(Channels2Merge)
                                ...
                                c=Channels2Merge(c1);
                                TempMaxY=max(TempMaxY,max(Channel_Info(c).Whole_Image_Values+(TraceCount*SplitTraceAdjust)));
                                TempMinY=min(TempMinY,min(Channel_Info(c).Whole_Image_Values+(TraceCount*SplitTraceAdjust)));
                                if ischar(Channel_Info(c).DisplayColorMap)
                                    if length(Channel_Info(c).DisplayColorMap)==1
                                        TempColor=Channel_Info(c).DisplayColorMap;
                                    else
                                ...
                                plot([1:length(Channel_Info(c).Whole_Image_Values)],...
                                    Channel_Info(c).Whole_Image_Values+(TraceCount*SplitTraceAdjust),...
                                    Trace_LineStyle,...
                                    'color',TempColor,'linewidth',MeanTrace_LineWidth)
                            end
                        else
                            TraceCount=TraceCount+1;
                            TempMaxY=max(TempMaxY,max(Channel_Info(Channel).Whole_Image_Values+(TraceCount*SplitTraceAdjust)));
                            TempMinY=min(TempMinY,min(Channel_Info(Channel).Whole_Image_Values+(TraceCount*SplitTraceAdjust)));
                            ...
                            plot([1:length(Channel_Info(Channel).Whole_Image_Values)],...
                                Channel_Info(Channel).Whole_Image_Values+(TraceCount*SplitTraceAdjust),...
                                MeanTrace_LineStyle,...
                                'color',TempColor,'linewidth',MeanTrace_LineWidth)
                        end
                    end
                    if SliceWholeImageTraces
                        if MergeChannel
                            %TempMaxY=-10000000000;
                            %TempMinY=10000000000;
                            for c1=1:length(Channels2Merge)
                                TraceCount=TraceCount+1;
                                c=Channels2Merge(c1);
                                TempMaxY=max(TempMaxY,max(Channel_Info(c).Slice(z0).Whole_Image_Values+(TraceCount*SplitTraceAdjust)));
                                TempMinY=min(TempMinY,min(Channel_Info(c).Slice(z0).Whole_Image_Values+(TraceCount*SplitTraceAdjust)));
                                if ischar(Channel_Info(c).DisplayColorMap)
                                    if length(Channel_Info(c).DisplayColorMap)==1
                                        TempColor=Channel_Info(c).DisplayColorMap;
                                    else
                                ...
                                plot([1:length(Channel_Info(c).Slice(z0).Whole_Image_Values)],...
                                    Channel_Info(c).Slice(z0).Whole_Image_Values+(TraceCount*SplitTraceAdjust),...
                                    SliceMeanTrace_LineStyle,...
                                    'color',TempColor,'linewidth',SliceMeanTrace_LineWidth)
                            end
                        else
                            TraceCount=TraceCount+1;
                            TempMaxY=max(TempMaxY,max(Channel_Info(Channel).Slice(z0).Whole_Image_Values+(TraceCount*SplitTraceAdjust)));
                            TempMinY=min(TempMinY,min(Channel_Info(Channel).Slice(z0).Whole_Image_Values+(TraceCount*SplitTraceAdjust)));
                            if ischar(Channel_Info(Channel).DisplayColorMap)
                                if length(Channel_Info(Channel).DisplayColorMap)==1
                                    TempColor=Channel_Info(Channel).DisplayColorMap;
                                else
                            ...
                            plot([1:length(Channel_Info(Channel).Slice(z0).Whole_Image_Values)],...
                                Channel_Info(Channel).Slice(z0).Whole_Image_Values+(TraceCount*SplitTraceAdjust),...
                                SliceMeanTrace_LineStyle,...
                                'color',TempColor,'linewidth',SliceMeanTrace_LineWidth)
                        end
                    end
                ...
                if ~WholeImageTraces&&~SliceWholeImageTraces&&isempty(ROIs)
                    TempMaxY=1;
                    TempMinY=0;
                end
                ...
                xlabel('Frame')
                switch TraceParam.TraceType
                    case 1
                        ylabel({TraceParam.TraceTypeLabel})
                    case 2
                        ylabel({TraceParam.TraceTypeLabel;TraceParam.F0_MethodLabel})
                    case 3
                        ylabel({TraceParam.TraceTypeLabel;TraceParam.F0_MethodLabel})
                end
                ylim(TraceYLimits)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function DisplayWholeImageTraces(~,~,~)
            WholeImageTraces=get(WholeImageTraces_Button,'value');
            set(WholeImageTraces_Button, 'Enable', 'off');
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            TraceDisplay([0,Last_T],Channel,Frame,Slice,ViewerFig,TracePlotAxis,TraceAxisPosition,0)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            set(WholeImageTraces_Button, 'Enable', 'on');
            set(ViewerFig,'CurrentObject',ViewerImageAxis)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
        function DisplaySliceWholeImageTraces(~,~,~)
            SliceWholeImageTraces=get(SliceWholeImageTraces_Button,'value');
            set(SliceWholeImageTraces_Button, 'Enable', 'off');
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            TraceDisplay([0,Last_T],Channel,Frame,Slice,ViewerFig,TracePlotAxis,TraceAxisPosition,0)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            set(SliceWholeImageTraces_Button, 'Enable', 'on');
            set(ViewerFig,'CurrentObject',ViewerImageAxis)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
                ...
                try
                    ...
                    if isempty(TraceScaleBars)
                        ...
                        switch TraceParam.TraceType
                            ...
                            case 2
                                TraceScaleBars.VertUnit={TraceParam.TraceTypeLabel};
                            case 3
                                TraceScaleBars.VertUnit={TraceParam.TraceTypeLabel};
                        end
                    ...
                    if ~isfield(TraceScaleBars,'VertUnit')
                        switch TraceParam.TraceType
                            case 1
                                TraceScaleBars.VertUnit='AU';
                            case 2
        ...
        function AddPixelTrace(~,~,~)
            ...
            if TileChannels||TileSlices||TileFrames
                ...
                for i=1:size(TempX,1)
                    ...
                    [ROIs(NumROIs).TraceChannel]=ExtractTraces(TraceParam,ROIs(NumROIs).Type,ROIs(NumROIs).Coord,[],ROIs(NumROIs).Slice);
                end
...
%                     end
                    if any(TempDataRegion(:))
                        ...
                        TempProps=regionprops(TempDataRegion,'Centroid');
                        ROIs(NumROIs).Coord=TempProps.Centroid;
                        [ROIs(NumROIs).BorderLine]=FindROIBorders(ROIs(NumROIs).DataRegion,1);
                        [ROIs(NumROIs).TraceChannel]=ExtractTraces(TraceParam,ROIs(NumROIs).Type,[],ROIs(NumROIs).DataRegion,ROIs(NumROIs).Slice);
                    else
                        cont=0;
                    end
                end
        ...
        function DeleteROI(~,~,~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            set(Delete_ROI_Button, 'Enable', 'off');
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            axes(ViewerImageAxis)
            Txt1=text(ZoomDataRegion_Props.BoundingBox(1)+ZoomDataRegion_Props.BoundingBox(3)*0.05,...
                ZoomDataRegion_Props.BoundingBox(2)+ZoomDataRegion_Props.BoundingBox(4)*0.05,...
                'Select ROI to Edit','color','w','fontsize',16);
            drawnow
            FindCoord=ginput(1);
            delete(Txt1);
            if ~isfield(ROIs,'Coord')
                for ROI1=1:length(ROIs)
                    if ROIs(ROI1).Type==1
                    else
                        TempProps=regionprops(ROIs(ROI1).DataRegion,'Centroid');
                        ROIs(ROI1).Coord=TempProps.Centroid;
            ...
            for ROI1=1:length(ROIs)
                TempDistances(ROI1)=...
                    sqrt((ROIs(ROI1).Coord(1)-FindCoord(1))^2+(ROIs(ROI1).Coord(2)-FindCoord(2))^2);
            end
            [~,ROI2Delete] = min(TempDistances);
            warning(['Removing ROI #',num2str(ROI2Delete)]);
            ROIs(ROI2Delete)=[];
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %UpdateDisplay
            CurrentImages=FindCurrentImage(Channel,Frame,Slice,StackOrder,Z_Projection,T_Projection,MergeChannel,0,TileChannels,TileSlices,TileFrames,TileSettings);
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            set(Delete_ROI_Button, 'Enable', 'on');
            set(ViewerFig,'CurrentObject',ViewerImageAxis)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if WasPlaying
                StartPlayStack(PlayButton);
        ...
        function EditROI(~,~,~)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            set(Edit_ROI_Button, 'Enable', 'off');
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            axes(ViewerImageAxis)
            Txt1=text(ZoomDataRegion_Props.BoundingBox(1)+ZoomDataRegion_Props.BoundingBox(3)*0.05,...
                ZoomDataRegion_Props.BoundingBox(2)+ZoomDataRegion_Props.BoundingBox(4)*0.05,...
            ...
            [~,ROI2Edit] = min(TempDistances);
            warning(['Editing ROI #',num2str(ROI2Edit)]);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if TileChannels||TileSlices||TileFrames
                WarningPopup = questdlg({'Please Exit Tiling Mode to Add ROI Traces'},'Problem Encountered!','OK','OK');
            else
                switch ROIs(ROI2Edit).Type
                    case 1
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        axes(ViewerImageAxis)
                        Txt1=text(ZoomDataRegion_Props.BoundingBox(1)+ZoomDataRegion_Props.BoundingBox(3)*0.05,...
                            ZoomDataRegion_Props.BoundingBox(2)+ZoomDataRegion_Props.BoundingBox(4)*0.05,...
                            'Select new ROI Center','color','w','fontsize',16);
                        drawnow
                        [TempX,TempY] = ginput(1);
                        for i=1:size(TempX,1)
                            ROIs(ROI2Edit).Type=1;
                            ROIs(ROI2Edit).Slice=Slice;
                            ROIs(ROI2Edit).Channel=Channel;
                            ROIs(ROI2Edit).Frame=Frame;
                            ROIs(ROI2Edit).Coord=round([TempX(i),TempY(i),Frame]);
                            [ROIs(ROI2Edit).TraceChannel]=ExtractTraces(TraceParam,ROIs(ROI2Edit).Type,ROIs(ROI2Edit).Coord,[],ROIs(ROI2Edit).Slice);
                        end
                        if T_Stack
                            axes(TracePlotAxis)
                        end
                    ...
                    case 2
                        ...
                        Txt1=text(ZoomDataRegion_Props.BoundingBox(1)+ZoomDataRegion_Props.BoundingBox(3)*0.05,...
                            ZoomDataRegion_Props.BoundingBox(2)+ZoomDataRegion_Props.BoundingBox(4)*0.05,...
                            'Select new ROI location','color','w','fontsize',16);
                        drawnow
                        cont=1;
                        Count=1;
                        TempDataRegion=roipoly;
    ...
    %                     end
                        if any(TempDataRegion(:))
                            ROIs(ROI2Edit).Type=2;
                            ROIs(ROI2Edit).Slice=Slice;
                            ROIs(ROI2Edit).Channel=Channel;
                            ROIs(ROI2Edit).Frame=Frame;
                            ROIs(ROI2Edit).DataRegion=TempDataRegion;
                            TempProps=regionprops(TempDataRegion,'Centroid');
                            ROIs(ROI2Edit).Coord=TempProps.Centroid;
                            [ROIs(ROI2Edit).BorderLine]=FindROIBorders(ROIs(ROI2Edit).DataRegion,1);
                            [ROIs(ROI2Edit).TraceChannel]=ExtractTraces(TraceParam,ROIs(ROI2Edit).Type,[],ROIs(ROI2Edit).DataRegion,ROIs(ROI2Edit).Slice);
                        else
                            cont=0;
                        end
                        delete(Txt1);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [TraceChannel]=ExtractTraces(TraceParam,ROIType,Coord,DataRegion,Slice)
            f = waitbar(0,['Extracting ROI Traces...']); 
            ...
            for cc=1:Last_C
                ...
                for tt=1:Last_T
                    ...
                    if ROIType==1
                        TempVal=TempImage1(Coord(2),Coord(1));
                    else
                        TempVal=nanmean(TempImage1(DataRegion));
                    end
                    TempTrace(tt)=TempVal;
                    if TraceParam.TraceType~=1
                        if TraceParam.F0_Method==1||TraceParam.F0_Method==2
                            if any(tt==TraceParam.F0_Frames)
                                TempBaseline=cat(3,TempBaseline,TempImage1);
                            end
                        elseif TraceParam.F0_Method==3||TraceParam.F0_Method==4
                            TempBaseline=cat(3,TempBaseline,TempImage1);
                        end
                    end
                     waitbar(tt/Last_T,f,['Extracting ROI Traces...']);
                end
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if TraceParam.TraceType~=1
                    switch TraceParam.F0_Method
                        case 1
                            TempBaseline=mean(TempBaseline,3);
                        case 2
                            TempBaseline=min(TempBaseline,3);
                        case 3
                            TempBaseline=movmean(double(TempBaseline),[TraceParam.F0_FrameWidth],3);
                        case 4
                            TempBaseline=movmin(double(TempBaseline),[TraceParam.F0_FrameWidth],3);
                    end
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    if TraceParam.F0_Method==1||TraceParam.F0_Method==2
                        if ROIType==1
                            TempBaselineVals=TempBaseline(Coord(2),Coord(1),:);
                        else
                            for ttt=1:size(TempBaseline,3)
                                TempImage0=TempBaseline(:,:,ttt);
                                TempBaselineVals(ttt)=nanmean(TempImage0(DataRegion));
                            end
                        end
                        TempBaselineVal=mean(TempBaselineVals(:));
                        if TraceParam.TraceType==2
                            TempTrace=TempTrace-TempBaselineVal;
                        elseif TraceParam.TraceType==3
                            TempTrace=(TempTrace-TempBaselineVal)/TempBaselineVal;
                        end
                    elseif TraceParam.F0_Method==3||TraceParam.F0_Method==4
                        if ROIType==1
                            TempBaselineVals=TempBaseline(Coord(2),Coord(1),:);
                        ...
                        if TraceParam.TraceType==2
                            for ttt=1:length(TempTrace)
                                TempTrace(ttt)=TempTrace(ttt)-TempBaselineVals(ttt);
                            end
                        elseif TraceParam.TraceType==3
                            for ttt=1:length(TempTrace)
                                TempTrace(ttt)=(TempTrace(ttt)-TempBaselineVals(ttt))/TempBaselineVals(ttt);
                            end
                        end
                    end    
                end
                ...
                clear TempTrace TempBaseline TempBaselineVals TempImage0 TempBaselineVal
            end
            waitbar(1,f,['Finished!']);
            close(f)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function TraceJump2Frame(~,~,~)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            set(TraceJumpButton, 'Enable', 'off');
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            axes(TracePlotAxis)
            FindCoord=ginput(1);
            Frame=round(FindCoord(1))
            set(Frame_sld,'Value',Frame)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %UpdateDisplay
            CurrentImages=FindCurrentImage(Channel,Frame,Slice,StackOrder,Z_Projection,T_Projection,MergeChannel,0,TileChannels,TileSlices,TileFrames,TileSettings);
...
%             [ViewerFig,ViewerImageAxis,ViewerImageAxisPosition,MaskAxes,TileAxes]=ImageDisplay(Channel,Frame,Slice,CurrentImages,ViewerFig,ViewerImageAxis,ViewerImageAxisPosition,MaskAxes,TileAxes,ViewerFigPosition);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [CurrentPosition,CurrentLabel]=GetCurrentPosition(ImagingInfo,ChannelSettings,SliceSettings,FrameSettings,ImageLabel,c,c1,z,z1,t,t1)
            ...
            if isempty(ChannelSettings)
                ...
                elseif any(c==[-11:-1:-19])
                    if isempty(Z_ProjectionSettings.Z_ProjectionType)
                        ChannelSettings.C_Display{c1}=[Channel_Labels{abs(c+10)},' Z Proj'];
                    else
                        ChannelSettings.C_Display{c1}=[Channel_Labels{abs(c+10)},' ',Z_ProjectionSettings.Z_ProjectionType,' Z Proj'];
                    end
                elseif c==-20
                ...
                elseif any(c==[-21:-1:-29])
                    if isempty(T_ProjectionSettings.T_ProjectionType)
                        ChannelSettings.C_Display{c1}=[Channel_Labels{abs(c+20)},' T Proj'];
                    else
                        ChannelSettings.C_Display{c1}=[Channel_Labels{abs(c+20)},' ',T_ProjectionSettings.T_ProjectionType,' T Proj'];
                    end
                end
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if isempty(SliceSettings)||~isfield(SliceSettings,'Z_Display')
                if z~=0
                    ...
                    if isempty(Z_ProjectionSettings.Z_ProjectionType)
                        ...
                        SliceSettings.Z_Display{z1}=['Z Projection'];
                    end
                end
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if isempty(FrameSettings)||~isfield(FrameSettings,'T_Display')
                if t~=0
                    if ~isnan(ImagingInfo.InterFrameTime)
                        ...
                        elseif TempFrameDecimals==length(TempFrameInterval)-4
                            TempT=round((t-1)*ImagingInfo.InterFrameTime*10000)/10000;
                            TempT=num2str(TempT);
                            TempDecimals=strfind(TempT,'.');
                            if isempty(TempDecimals)
                                TempT=[TempT,'.0000'];
                            ...
                            elseif TempDecimals==length(TempT)-2
                                TempT=[TempT,'00'];
                            elseif TempDecimals==length(TempT)-3
                                TempT=[TempT,'0'];
                            end
                        else
                            TempT=0;
                    ...
                    if isempty(T_ProjectionSettings.T_ProjectionType)
                        FrameSettings.T_Display{t1}=[T_ProjectionSettings.T_ProjectionType,' T Projection'];
                    else
        ...
        %                 end
                ...
                %end
                if Z_Stack&&ImageLabel.IncludeZ&&c~=-10
                    CurrentLabel=[CurrentLabel,SliceSettings.Z_Display{z1},' '];
                end
                if T_Stack&&ImageLabel.IncludeT&&c~=-20
    ...
    %             end
                ...
                warning('Problem updating current position...')
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if c==0&&z~=0&&t~=0
            ...
            elseif c==-10
                ...
                CurrentPosition.TileSettings=0;
            elseif any(c==[-11:-1:-19])
                CurrentPosition.Channel=abs(c+10);
                CurrentPosition.Frame=t;
                CurrentPosition.Slice=1;
                CurrentPosition.Z_Projection=1;
                CurrentPosition.T_Projection=0;
            ...
            elseif c==-20
                ...
                CurrentPosition.TileSettings=0;
            elseif any(c==[-21:-1:-29])
                CurrentPosition.Channel=abs(c+20);
                CurrentPosition.Frame=1;
                CurrentPosition.Slice=z;
                CurrentPosition.Z_Projection=0;
                CurrentPosition.T_Projection=1;
                ...
                while ExportSetup
                    ...
                    if ~RGB_Stack
                        if ColorBarOverlayOn
                            ...
                            TempChoice = questdlg({'Add ColorBar Overlay?'},'Add ColorBar Overlay?','Add','Skip','Skip');
                            switch TempChoice
                                case 'Add'
                                    AdjustColorBarOverlay=1;
                                case 'Skip'
...
%                                 end
                                ...
                                if ExportSettings.ExportIncludeTrace
                                    ...
                                    SubPlotAxisPos(PanelCount).Position=get(SubPlotAxis(PanelCount),'position');
                                    TraceRange=[ExportSettings.T_Range(1),ExportSettings.T_Range(length(ExportSettings.T_Range))];
                                    TraceRange=[1,Last_T];
                                    TraceDisplay(TraceRange,ExportSettings.TraceChannel,t0,ExportSettings.TraceSliceSelection.Z_Range,ExportFig,SubPlotAxis(PanelCount),SubPlotAxisPos(PanelCount).Position,ExportSettings.SplitTraceAdjust);
                                    if ~TraceScaleOn
                                        set(SubPlotAxis(PanelCount),'visible','on');
                                    end
                                end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [ChannelSettings]=ChannelSelection(ChannelSettings)
            if C_Stack
                ...
                if ~RGB_Stack
                    ...
                    ChannelSettings.C_Option_Labels=Channel_Labels;
                    ChannelSettings.C_Options=[1:Last_C];
                end
                if Z_Stack&&~RGB_Stack
                    for ccc=1:Last_C
                        if ~isempty(Z_ProjectionSettings.Z_ProjectionType)
                            ChannelSettings.C_Option_Labels=horzcat(ChannelSettings.C_Option_Labels,[Channel_Labels{ccc},' ',Z_ProjectionSettings.Z_ProjectionType,' Z Project']);
                        else
                            ChannelSettings.C_Option_Labels=horzcat(ChannelSettings.C_Option_Labels,[Channel_Labels{ccc},' Z Project']);
                        end
                        ChannelSettings.C_Options=[ChannelSettings.C_Options,-10-ccc];
                    end
                    if ~isempty(Z_ProjectionSettings.Z_ProjectionType)
                    ...
                    ChannelSettings.C_Options=[ChannelSettings.C_Options,-10];
                end
                if T_Stack&&~RGB_Stack
                    for ccc=1:Last_C
                        if ~isempty(T_ProjectionSettings.T_ProjectionType)
                            ChannelSettings.C_Option_Labels=horzcat(ChannelSettings.C_Option_Labels,[Channel_Labels{ccc},' ',T_ProjectionSettings.Z_ProjectionType,' T Project']);
                        else
                            ChannelSettings.C_Option_Labels=horzcat(ChannelSettings.C_Option_Labels,[Channel_Labels{ccc},' T Project']);
                        end
                        ChannelSettings.C_Options=[ChannelSettings.C_Options,-20-ccc];
                    end
                    if ~isempty(T_ProjectionSettings.T_ProjectionType)
                ...
                if MergeChannel&&~isempty(Z_Projection_Data)&&~isempty(T_Projection_Data)&&~RGB_Stack
                    DefaultChoices=[1:Last_C+2+Last_C*2];
                elseif MergeChannel&&~isempty(Z_Projection_Data)&&isempty(T_Projection_Data)&&~RGB_Stack
                    DefaultChoices=[1:Last_C+2+Last_C];
                elseif MergeChannel&&isempty(Z_Projection_Data)&&~isempty(T_Projection_Data)&&~RGB_Stack
                    DefaultChoices=[1:Last_C+2+Last_C];
                elseif MergeChannel&&~RGB_Stack
                    DefaultChoices=[1:Last_C+1];
                elseif ~MergeChannel&&length(Channel_Colors)>1&&~RGB_Stack
                    DefaultChoices=[1:Last_C+1];
                else
                ...
                if length(ChannelSettings.C_Range)>1
                    ...
                    switch AdjustOrder
                        case 'Adjust'
                            ...
                            prompt = {'C_Range Order (0 Merge, -10 Merge Z Proj, -20 Merge T Proj)'};
                ...
                for c1=1:length(ChannelSettings.C_Range)
                    ...
                    if c~=0&&~any(c==[-10:-1:-19])&&~any(c==[-20:-1:-29])
                        ChannelSettings.C_Display{c1}=Channel_Labels{c};
                    elseif c==0
                        ChannelSettings.C_Display{c1}=['Merge'];
                    elseif c==-10
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if any(ChannelSettings.C_Range<=-10)&&any(ChannelSettings.C_Range>=-19)&&Z_Projection==0
                Z_Projection=1;
                set(Z_ProjectionButton,'value',Z_Projection);
                Z_ProjectData
                Merge_Z_Projection
            ...
            if any(ChannelSettings.C_Range<=-20)&&any(ChannelSettings.C_Range>=-29)&&T_Projection==0
                T_Projection=1;
                set(T_ProjectionButton,'value',T_Projection);
                T_ProjectData
                Merge_T_Projection
        ...
        function [FrameSettings]=FrameSelection(FrameSettings)
            if T_Stack
                ...
                for tt=1:Last_T
                    FrameSettings.T_Option_Labels{tt}=num2str(tt);
                end
                if ~isempty(T_ProjectionSettings)
                    if ~isfield(Z_ProjectionSettings,'T_ProjectionType')
                        T_ProjectionSettings.T_ProjectionType=[];
                    end
                    FrameSettings.T_Option_Labels=horzcat(FrameSettings.T_Option_Labels,[T_ProjectionSettings.T_ProjectionType,' T Projection']);
                else
                ...
                for t1=1:length(FrameSettings.T_Range)
                    ...
                    if t~=0
                        if ~isfield(ImagingInfo,'InterFrameTime')
                            ImagingInfo.InterFrameTime=1;
                        end
                        if ~isfield(ImagingInfo,'FrameUnit')
                            ImagingInfo.FrameUnit='frame';
                        end
                        if ~isnan(ImagingInfo.InterFrameTime)
                            TempFrameInterval=num2str(ImagingInfo.InterFrameTime);
                            TempFrameDecimals=strfind(TempFrameInterval,'.');
