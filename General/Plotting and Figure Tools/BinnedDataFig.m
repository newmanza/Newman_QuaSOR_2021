...
if ~exist('Overwrite_MarkerStyle')
    ClearOverWriteMarker=1;
    Overwrite_MarkerStyle=zeros(1,size(XVals_Mean,2));
else
    ClearOverWriteMarker=0;
end
if size(Bin_Colors,1)==size(XVals_Mean,1)&&size(Bin_Colors,2)==size(XVals_Mean,2)
    ColorMode=3;
    AllColors=[];
    LineColors=[];
    for r=1:size(XVals_Mean,2)
        for q=1:size(XVals_Mean,1)
            AllColors{q,r}=Bin_Colors{q,r};
            LineColors{r}=Bin_Colors{1,r};
        end
    end
else
...
%                             SilentMarkerLines{q},'MarkerSize',SilentMarkerSizes(q),'LineWidth',0.5,'color',AllColors{q,r})
            if Overwrite_MarkerStyle(r)
                errorbar(   XVals_Mean(q,r),...
                            ...
                            XVals_Error(q,r),...
                            XVals_Error(q,r),...
                            [Overwrite_MarkerStyles{r},'-'],'MarkerSize',Overwrite_MarkerSize{r},'LineWidth',StandardLineWidth,'color',AllColors{q,r})
            else
                if any(q==SilentBins)
                    errorbar(   XVals_Mean(q,r),...
                                YVals_Mean(q,r),...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
try
    Full_Export_Fig(gcf,gca,Check_Dir_and_File(BinnedSaveDir,FigName,[],1),1)
catch
    Full_Export_Fig(gcf,gca,Check_Dir_and_File(BinnedSaveDir,FigName,[],1),3)
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ClearOverWriteMarker
    clear Overwrite_MarkerStyle
end