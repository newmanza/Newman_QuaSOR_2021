function handles = barweb1(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
    ...
    %barvalues;
    %errors;
	handles.bars = bar([1:numgroups],barvalues, width,'edgecolor','k', 'linewidth', 0.75);
	hold on
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', 'NorthEast', 'fontsize',14);
		legend boxoff;
    else
		handles.legend = [];
    end
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)-length(xdata)/2
				text(xdata(j),  -0.01*ymax*1.1, bw_legend(i), 'Rotation', 30, 'fontsize', 10, 'HorizontalAlignment', 'right');
			end
		end
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',10,'Interpreter','none');
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',10);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',10);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 10, 'xtick',1:numgroups, 'linewidth', 0.5,'xgrid','off','ygrid','off');
