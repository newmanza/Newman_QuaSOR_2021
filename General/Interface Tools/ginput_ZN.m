function varargout = ginput_ZN(varargin)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    initialState = setupFcn(fig,SelectionColor,p);
    ...
    while how_many ~= 0
        try
            mode = waitForUserInput(fig,SelectionColor,p);
        catch %#ok<CTCH>
            cleanup(c,p);
            if(ishghandle(fig))
        ...
        if (isCorrectFigure(fig,SelectionColor,p))
            switch mode
                case 'key'
            ...
            p(length(p)+1)=plot(x,y,'+','color','k','markersize',7);
            p(length(p)+1)=plot(x,y,'o','color','k','markersize',7);
            p(length(p)+1)=plot(x,y,'+','color',SelectionColor,'markersize',6);
            p(length(p)+1)=plot(x,y,'o','color',SelectionColor,'markersize',6);
...
function mode = waitForUserInput(fig,SelectionColor,p)
...
function initialState = setupFcn(fig,SelectionColor,p)
...
initialState.CrossHair = createCrossHair(fig,SelectionColor,p);
...
function crossHair = createCrossHair(fig,SelectionColor,p)
...
function valid = isCorrectFigure(fig,SelectionColor,p)
