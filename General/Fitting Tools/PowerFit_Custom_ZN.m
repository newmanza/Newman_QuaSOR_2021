function DataStructure=PowerFit_Custom_ZN(Xdata,Ydata,Power,FitOption)
    ...
    elseif size(Xdata,1)~=size(Ydata,1)||size(Xdata,2)~=size(Ydata,2)
        ...
        switch FitOption
            case 1
                DataStructure.FitType='Power-0';
            case 2
                DataStructure.FitType='Power';
        end
        try
            ...
            switch FitOption
                ...
                case 2
                    ...
                    if DataStructure.Coefficients(2)<0
                        DataStructure.FitEquation_Text = strcat('y=',num2str(DataStructure.Coefficients(1)),...
                            'x^{',num2str(DataStructure.Coefficients(2)),'} ',num2str(DataStructure.Coefficients(2)));
        ...
        %     DataStructure.CorrCoef_P=DataStructure.Correlation_p(1,2);
            ...
            switch FitOption
                case 1
                    ...
                    if abs(Power)>=1
                        Power=round(Power);
                    elseif abs(Power)<1&&abs(Power)>=0.1
                        Power=round(Power*10)/10;
                    elseif abs(Power)<0.1&&abs(Power)>=0.01
                        Power=round(Power*100)/100;
                    elseif abs(Power)<0.01&&abs(Power)>=0.001
                        Power=round(Power*1000)/1000;
                    elseif abs(Power)<0.001&&abs(Power)>=0.0001
                        Power=round(Power*10000)/10000;
                    elseif abs(Power)<0.0001&&abs(Power)>=0.00001
                        Power=round(Power*100000)/100000;
                    end
                ...
                case 2
                    ...
                    Temp_b=DataStructure.Coefficients(2);
                    if abs(Temp_b)>=1
                        Temp_b=round(abs(Temp_b));
                    elseif abs(Temp_b)<1&&abs(Temp_b)>=0.1
                        Temp_b=round(abs(Temp_b*10)/10);
                    elseif abs(Temp_b)<0.1&&abs(Temp_b)>=0.01
                        Temp_b=round(abs(Temp_b*100)/100);
                    elseif abs(Temp_b)<0.01&&abs(Temp_b)>=0.001
                        Temp_b=round(abs(Temp_b*1000)/1000);
                    elseif abs(Temp_b)<0.001&&abs(Temp_b)>=0.0001
                        Temp_b=round(abs(Temp_b*10000)/10000);
                    elseif abs(Temp_b)<0.0001&&abs(Temp_b)>=0.00001
                        Temp_b=round(abs(Temp_b*100000)/100000);
                    end
                    if DataStructure.Coefficients(2)<0
                        DataStructure.CorrCoef_PValue_Text = ...
                            [num2str(Temp_a),...
