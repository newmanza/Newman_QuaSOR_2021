function StatsTableExporterFull(Statistics,PvaluesTable,OverallPValue,GroupNames,TestName,SaveDir)
    OS=computer;
    if strcmp(OS,'MACI64')
        dc='/';
    else
    ...
    if iscell(Statistics)
        for i=1:length(Statistics)
            if Statistics{i}.Num>0
                BasicStatsCell{i}=[num2str(i),': n = ',num2str(Statistics{i}.Num),' Mean ',num2str(Statistics{i}.Mean),...
                    ' STD ',num2str(Statistics{i}.STD),' SEM ',num2str(Statistics{i}.SEM),' CV ',num2str(Statistics{i}.CV),...
                    ' Min ',num2str(Statistics{i}.Min),' Max ',num2str(Statistics{i}.Max),' Med ',num2str(Statistics{i}.Median)];
            else
                BasicStatsCell{i}=[num2str(i),': n = ',num2str(Statistics{i}.Num)];
            end
        end
    else
        if Statistics.Num>0
            BasicStatsCell{1}=[num2str(i),': n = ',num2str(Statistics.Num),' Mean ',num2str(Statistics.Mean),...
                ' STD ',num2str(Statistics.STD),' SEM ',num2str(Statistics.SEM),' CV ',num2str(Statistics.CV),....    
                ' Min ',num2str(Statistics.Min),' Max ',num2str(Statistics.Max),' Med ',num2str(Statistics.Median)];
        else
            BasicStatsCell{i}=[num2str(i),': n = ',num2str(Statistics.Num)];
        end
    end
...
%         end
    ...
    EmptyRow=cell(1,size(PvaluesTable,1)+1);
    ...
    for i=1:length(BasicStatsCell)
        Temp=EmptyRow;
        Temp{1,1}=BasicStatsCell{i};
        FinalTable=vertcat(FinalTable,Temp);
        clear Temp
    end
    if ~isempty(SaveDir)
        FinalTable=vertcat(FinalTable,Spacer);
        ...
        dlmcell([SaveDir, dc ,TestName , ' Statistics Summary' , '.txt'], FinalTable,'delimiter','\t');
    else
        warning on
        warning('NO DIRECTORY PROVIDED!')
    end
end