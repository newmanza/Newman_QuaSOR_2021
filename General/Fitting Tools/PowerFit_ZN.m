function DataStructure=PowerFit_ZN(Xdata,Ydata,FitOption)
    ...
    elseif size(Xdata,1)~=size(Ydata,1)||size(Xdata,2)~=size(Ydata,2)
        ...
        switch FitOption
            ...
            case 3
                DataStructure.FitType='Power-0 Alt';
            case 4
                DataStructure.FitType='Power Alt';
        end
        try
            DataStructure.FitXValues=[];
            DataStructure.FitYValues=[];
            DataStructure.FitEquation_Text=[];
            ...
            elseif FitOption==4
                ...
                DataStructure.Coefficients(1) = DataStructure.Power_Fit_Model.a;
                DataStructure.Coefficients(2) = DataStructure.Power_Fit_Model.b;
                DataStructure.Coefficients(3) = DataStructure.Power_Fit_Model.c;
            end
            ...
            switch FitOption
                case 1
                    for count=1:length(TempX) 
                        TempY(count)=DataStructure.Coefficients(1) * TempX(count).^DataStructure.Coefficients(2);
                    end
                    fixvals=0;
                    for count=1:length(TempX) 
                    ...
                    for count=1:length(Xdata) 
                        DataStructure.yfit(count) = DataStructure.Coefficients(1) * Xdata(count) .^ DataStructure.Coefficients(2);
                    end
                    DataStructure.FitEquation_Text = strcat('y=',num2str(DataStructure.Coefficients(1)),...
                        'x^{',num2str(DataStructure.Coefficients(2)),'}');
                case 2
                    for count=1:length(TempX) 
        ...
        %     DataStructure.CorrCoef_P=DataStructure.Correlation_p(1,2);
            ...
            switch FitOption
                ...
                case 2
                    ...
                    Temp_c=DataStructure.Coefficients(3);
                    if abs(Temp_c)>=1
