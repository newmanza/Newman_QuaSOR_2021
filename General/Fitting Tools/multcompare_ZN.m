function [comparison,means,h,gnames] = multcompare_ZN(stats,varargin)
...
% -----------------------------------------------
function crit = getcritOLD(ctype, alpha, df, ng)
...
while(~isempty(onetype))
   ...
   switch onetype
    case 'tukey-kramer' % or hsd
     ...
     pval = 0*t;
     for j=1:numel(t)
         pval(j) = 1 - stdrcdf1(sqrt(2)*abs(t(j)),df,ng);%(q, v, r, upper)
     end
