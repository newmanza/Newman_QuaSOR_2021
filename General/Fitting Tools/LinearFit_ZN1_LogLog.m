function DataStructure=LinearFit_ZN1_LogLog(Xdata,Ydata)
    %Changes XFit Data so no Zero!
    ...
    elseif size(Xdata,1)~=size(Ydata,1)||size(Xdata,2)~=size(Ydata,2)
        ...
        TempMinX=min(TempXData_NaN(:));
        TempMinX_Log=log10(TempMinX);
        TempMinX_Log=floor(TempMinX_Log);
        TempMaxX=TempMaxX+0.1*TempMaxX;

        FitIncrement=TempMaxX/100;

        DataStructure.FitXValues=(10^TempMinX_Log):FitIncrement:TempMaxX;
