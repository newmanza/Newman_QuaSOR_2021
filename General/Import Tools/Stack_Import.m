...
%     MetaData,Additional_MetaData,BasicFileData,Channel_Labels,Channel_Colors,Channel_Color_Codes]=...
%     Stack_Import(FileName,LoadDir,ScratchDir,ChannelSelection,TurboMode,MetaData,MetaDataLoadingMethod)
% 
% 
    TurboMode=0;
    RawDataDir=[];
    ...
    switch nargin
        ...
        case 7
            ...
            MetaDataLoadingMethod=varargin{7};
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~exist('imreadbf.m')
        error('Please Install OME-Bioformats and imreadbf https://docs.openmicroscopy.org/bio-formats/6.1.0/users/matlab/index.html and https://www.mathworks.com/matlabcentral/fileexchange/32920-imread-for-multiple-life-science-image-file-formats')
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if isempty(MetaDataLoadingMethod)
        MetaDataLoadingMethod=1;
    end
    MetaDataLoading(MetaDataLoadingMethod);
    ...
    MetaData.AllTimeStamps=AllTimeStamps;
    MetaData.AllFrameTimes=AllFrameTimes;
    ...
    switch FileType
        case 1
            ...
            switch StackOrder
                case 'YX'
                    ImageArray=zeros(StackSizeY,StackSizeX,DataClass);
                    slice=1;
                    frame=1;
                    ...
                    clear TempImage
                case 'YXT'
                    ImageArray=zeros(StackSizeY,StackSizeX,StackSizeT,DataClass);
                    slice=1;
                    frame=1;
                    TotalNumImages=StackSizeT;
                    for i=1:TotalNumImages
                        TempImage=imread([LoadDir,dc,FileName],i);
                        switch DataClass
                            case 'double'
                                ImageArray(:,:,frame)=double(TempImage);
                            case 'single'
                                ImageArray(:,:,frame)=single(TempImage);
                            case 'uint16'
                                ImageArray(:,:,frame)=uint16(TempImage);
                            case 'uint8'
                                ImageArray(:,:,frame)=uint8(TempImage);
                        end
                        clear TempImage
                        frame=frame+1;
...
%             clear TempImageArray
    ...
    function MetaDataLoading(MetaDataLoadingMethod)
        %Load Meta Data
        ...
        if ~PreviousMetaData
            ...
            switch FileType
                case 1
                    ...
                    StackSizeT=length(imfinf);
                    StackSizeZ=1;
                    StackSizeC=1;
                    %XY Resolution
                    try
                        ScalingX=imfinf(1).XResolution;
                        ScalingY=imfinf(1).YResolution;
                        if strcmp(imfinf(1).ResolutionUnit,'Centimeter')
                    ...
                    %Frame Rate
                    try
                        for t=1:length(imfinf)
                            newStr = extractBefore(imfinf(t).ImageDescription,'acqTriggerTimestamps_sec');
                            newStr = extractAfter(newStr,'frameTimestamps_sec = ');
                            AllTimeStamps(t) = str2num(newStr);
                            if t>1
                                AllFrameTimes(t-1)=AllTimeStamps(t)-AllTimeStamps(t-1);
                            end
                        end
                        ScalingT=mean(AllFrameTimes);
                    catch
                    end
                    %Channels
                    try
                    ...
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    switch MetaDataLoadingMethod
                        case 1
                            try 
                                TIF_MetaDataLoading(1);
                            catch
                                warning on
                                warning('Problem with extracting metadata trying another option...')
                                TIF_MetaDataLoading(2);
                            end
                        case 2
                            try 
                                TIF_MetaDataLoading(2);
                    ...
                    if isempty(StackSizeX)
                        StackSizeX=1;
                    end
                    if isempty(StackSizeY)
                        StackSizeY=1;
                    end
                    if isempty(StackSizeZ)
                        StackSizeZ=1;
                    end
                    if isempty(StackSizeT)
                        StackSizeT=1;
                    end
                    if isempty(StackSizeC)
                        StackSizeC=1;
                    ...
                    if ~TurboMode
                        ...
                        while VerifySettings
                            ...
                            disp(['I found a total of : ',num2str(TotalImages),' Images in the file that need to be accounted for...'])
                            prompt = {'StackSizeX','StackSizeY',['StackSizeT(',num2str(TotalImages),')'],['StackSizeZ(',num2str(TotalImages),')'],['StackSizeC(',num2str(TotalImages),')'],...
    ...
    function TIF_MetaDataLoading(MetaDataLoadingMethod)
        switch MetaDataLoadingMethod
            case 1
                disp('Using bfGetReader and GetMetaData to access meta data')
            ...
            case 2
                ...
                for i=1:NumEntries
                    tempval=entries(i).toString;
                    if any(strfind(tempval,'Global Software=SI.ConfigPageClass'))
                        %SI.hStackManager.actualStackZStepSize = 0.5
                        tempstr = convertCharsToStrings(tempval.toCharArray);
                        tempstr=extractAfter(tempstr,'SI.hStackManager.actualStackZStepSize = ');
                        tempstr=extractBefore(tempstr,'SI');
                        ScalingZ=str2num(tempstr);
                        
                        %SI.hStackManager.numSlices = 21
                        tempstr = convertCharsToStrings(tempval.toCharArray);
                        tempstr=extractAfter(tempstr,'SI.hStackManager.numSlices = ');
                        tempstr=extractBefore(tempstr,'SI');
                        StackSizeZ=str2num(tempstr);
