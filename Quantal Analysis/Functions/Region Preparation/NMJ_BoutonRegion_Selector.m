function NMJ_BoutonRegion_Selector(StackSaveName,BoutonSelectionDisplayImage, AllBoutonsRegion1)
   OS=computer;
    if strcmp(OS,'MACI64')
        dc='/';
    else
...
%     end
    function BoutonArray1 = AddBoutons(BoutonArray1,AllBoutonsRegion1) % draw a region, this region will be added to the logical image
        hold on;
        Txt1=text(10, 10, 'Select Alignment Bouton Regions (Create Empty ROI to finish)','color','y','FontSize',20');
        cont = 1;     
        while cont
            ActiveSelection=1;
