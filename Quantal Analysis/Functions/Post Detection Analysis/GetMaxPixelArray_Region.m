...
function ImageArray_CorrAmp_norm_InMax = GetMaxPixelArray_Region(ImageArray_CorrAmp_norm, ImageArray_Labels, se);
...
for ImageNumber=1:LastImageNumber
    ...
    for LabelIndex=FirstLabelNum:LastLabelNum     
        ...
        OneImage_OneLabel_Mask_Large=imdilate(OneImage_OneLabel_Mask,se);
        
        % get image with one response, corresponding to current label
        
        MaskedImage=OneImage_CorrAmp_norm .* OneImage_OneLabel_Mask_Large;
...
%         
        % Output image
        OneImage_Corramp_norm_InMax = OneImage_Corramp_norm_InMax + (OneImage_OneResponse);       
