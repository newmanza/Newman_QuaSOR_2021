...
for n_dd=1:Numbder_dd
    ...
    if n_dd >1
        MatMinMax_New= (sign_Prev_deriv==sign(part_deriv)).*MatMinMax_New;
    else
        MatMinMax_New=sign(part_deriv);
    end

    sign_Prev_deriv=sign(part_deriv);
end

%Well , now the easy part, all values MatMinMax_New ==1 are local maximum and
%the values MatMinMax_New ==-1 are minimun

AllMaxima=find(MatMinMax_New==1);
AllMinima=find(MatMinMax_New==-1);
