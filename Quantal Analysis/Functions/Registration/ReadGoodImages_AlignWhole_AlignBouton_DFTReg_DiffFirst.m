...
function [ImageArray_Reg, DeltaX_DFTReg_Whole, DeltaY_DFTReg_Whole, DeltaX_DFTReg_Bouton, DeltaY_DFTReg_Bouton] = ReadGoodImages_AlignWhole_AlignBouton_DFTReg_DiffFirst(StackSaveName, StackFileName, GoodImages, FilterSize, FilterSigma, BoutonArray, Crop_Props, FirstImageFileName);
FlipLR=1;
...
PixelBorder=ones(6);%This adds buffer zone (10x10 works well most of the time) to boutons to avoid FFT artifacts, may cause problems with ROI too close to borders



LastImageNumber = GoodImages(length(GoodImages));
LastBoutonNumber = size(BoutonArray, 2);
FirstImage = imread(FirstImageFileName,'tif',GoodImages(1));
